...
export default function Header() {
  ...
  const fetchOnlineCategory = () => {
    axios
      .get(`${apiBaseUrl}/category/fetch-online-category`)
      .then((res) => res.data)
      .then((finalRes) => {
  ...
  const fetchOfflineCategory = () => {
    axios
      .get(`${apiBaseUrl}/category/fetch-offline-category`)
      .then((res) => res.data)
      .then((finalRes) => {
      ...
      <div className="lg:hidden block">
        <div className="flex items-center justify-between gap-0 py-[00px] px-2 shadow-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {cartData.length >= 1 && (
                <span className="text-gray-950 bg-white shadow-2xl border border-gray-300 text-[15px] w-6 rounded-full text-center absolute -top-3 -right-1">
                  {cartData?.length}
                </span>
              )}
            </button>
            ...
            {token ? (
              <button
                ...
                className="border-2 rounded-[10px] cursor-pointer border-gray-950 text-gray-950   capitalize duration-200 px-4 py-2 relative"
              >
                Logout
              </button>
            ) : (
        ...
        >
          ...
          <ul className="px-3">
            ...
            <li
              onClick={() => setSubCategoryMenu("Offline Course")}
              className="my-[25px] text-[22px] border-b-[1px] border-gray-100 pb-2 capitalize"
            >
              offline course
            </li>
            <li
              onClick={() => setSubCategoryMenu("Study Materials")}
              className="my-[25px] text-[22px] border-b-[1px] border-gray-100 pb-2 capitalize"
            >
              study materials
            </li>
            <li
              onClick={() => setSubCategoryMenu("Test Series")}
              className="my-[25px] text-[22px] border-b-[1px] border-gray-100 pb-2 capitalize"
            >
              test series
            </li>
            <li
              onClick={() => setSubCategoryMenu("Free Resources")}
              className="my-[25px] text-[22px] border-b-[1px] border-gray-100 pb-2 capitalize"
            >
              free resources
            </li>
            ...
            {subCategoryMenu == "Online Course" && (
              <ul className="">
                {onlineCourseData?.map((item, index) => {
                  return (
                    <Link
                      href={`/online-courses/${item?.courseName?.toLowerCase().replace(/[^a-zA-Z0-9]/g, "-")}`}
                    >
                      <li
                        onClick={() => {
                        ...
                        key={index}
                        className="shadow-xs border-[1px] border-gray-200 py-3 px-4 rounded-md bg-white capitalize text-gray-800 hover:bg-gray-100 mb-3 transition duration-200"
                      >
                        {item?.courseName}{" "}
                      </li>
                    </Link>
            ...
            {subCategoryMenu == "Offline Course" && (
              <ul className="">
                {offlineCourseData?.map((item, index) => {
                  return (
                    <Link
                      href={`/offline-courses/${item?.courseName?.toLowerCase().replace(/[^a-zA-Z0-9]/g, "-")}`}
                    >
                      <li
                        onClick={() => {
                          setSubCategoryMenu(false);
